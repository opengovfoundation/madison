...
```
$ mysqldump --user=username --password=password database_name > madison_db_dump.sql
```
...
```
$ mysql -u username -p password database_name < madison_db_dump.sql
```
...
configuration file [included in the docs](/docs/apache.conf.example).
...
command=php /path/to/madison/server/artisan queue:work sqs --sleep=3 --tries=3
--daemon
autostart=true
autorestart=true
user=forge
