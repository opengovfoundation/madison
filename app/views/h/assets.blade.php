<!-- H Styles -->
{{ HTML::style('vendor/h/css/inject.css') }}

<!-- H Scripts -->
{{ HTML::script('vendor/h/lib/annotator.js') }}
{{ HTML::script('vendor/h/lib/d3.js') }}
{{ HTML::script('vendor/h/lib/jschannel.js') }}
{{ HTML::script('vendor/h/lib/gettext.js') }}
{{ HTML::script('vendor/h/locale/data.js') }}
{{ HTML::script('vendor/h/lib/annotator.js') }}
{{ HTML::script('vendor/h/js/plugin/bridge.js') }}
{{ HTML::script('vendor/h/lib/annotator.document.js') }}
{{ HTML::script('vendor/h/js/plugin/heatmap.js') }}
{{ HTML::script('vendor/h/lib/annotator.texthighlights.js') }}
{{ HTML::script('vendor/h/js/dom_text_mapper.js') }}
{{ HTML::script('vendor/h/lib/annotator.domtextmapper.js') }}
{{ HTML::script('vendor/h/lib/annotator.textanchors.js') }}
{{ HTML::script('vendor/h/lib/diff_match_patch_uncompressed.js') }}
{{ HTML::script('vendor/h/js/text_match_engines.js') }}
{{ HTML::script('vendor/h/js/dom_text_matcher.js') }}
{{ HTML::script('vendor/h/lib/annotator.fuzzytextanchors.js') }}
{{ HTML::script('vendor/h/js/page_text_mapper_core.js') }}
{{ HTML::script('vendor/h/lib/annotator.pdf.js') }}
{{ HTML::script('vendor/h/js/plugin/toolbar.js') }}
{{ HTML::script('vendor/h/js/guest.js') }}
{{ HTML::script('vendor/h/js/host.js') }}
{{ HTML::script('vendor/h/')}}